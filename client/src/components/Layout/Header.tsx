...
import { Sun, Moon, Monitor, Menu, Bell, BellOff, Github, User, LogOut } from 'lucide-react';
...
export const Header = forwardRef<HTMLDivElement, HeaderProps>(
  ({ onMenuClick, onThemeToggle, theme, sidebarOpen }, ref) => {
    const { state, dispatch } = useApp();

    useImperativeHandle(ref, () => ({
      getHeight: () => ref.current?.offsetHeight || 60,
    }));
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'fixed top-0 left-0 right-0 z-30 h-16 bg-background/80 dark:bg-gray-950/80 backdrop-blur-md',
            ...
            <div className="absolute right-0 top-full mt-2 w-48 bg-background dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all py-1 z-50">
              {state.user ? (
                <>
                  ...
                  <button className="w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2 text-red-600">
