...
export function parseVoiceCommand(transcript: string): VoiceCommand {
  ...
  const intents: Record<string, VoiceIntent> = {
    ...
    'make image': 'generate_image',
    'analyze code': 'analyze_code',
    'review code': 'analyze_code',
    'refactor code': 'refactor_code',
    'improve code': 'refactor_code',
    'write tests': 'write_tests',
    'create tests': 'write_tests',
    'document code': 'document_code',
    'add comments': 'document_code',
    'search code': 'search_code',
    'find code': 'search_code',
    'go to file': 'navigate_file',
    'open file': 'navigate_file',
    'toggle': 'toggle_feature',
    'enable': 'toggle_feature',
    'disable': 'toggle_feature',
    'settings': 'settings',
  ...
  if (branchMatches) {
    entities.branchName = branchMatches[0].replace(/(?:branch|called|named)\s+["']?/i, '').replace(/["']?$/, '').trim();
  }
  ...
  return {
    transcript,
    confidence: matchedPhrase ? 0.8 : 0.3,
    intent,
    entities,
  };
}
...
export function extractImages(content: string): GeneratedImage[] {
  ...
  while ((match = regex.exec(content)) !== null) {
    images.push({
      id: generateId(),
      url: match[2],
      prompt: match[1] || 'Generated image',
      timestamp: new Date(),
    });
  }
